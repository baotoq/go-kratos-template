...
// Tag value mirrored from Secrets — kept here so the test breaks loudly if a
// tag is renamed without updating callers.
const keyRedisHost = "REDIS_HOST"
...
func TestLoadSecrets_Success(t *testing.T) {
	...
	assert.Equal(t, &Secrets{RedisHost: "redis:6379"}, secrets)
}
...
func TestLoadSecrets_MissingRequiredField(t *testing.T) {
	cases := []struct {
		...
		{name: "missing redis host", bundle: map[string]string{}},
		{name: "empty redis host", bundle: map[string]string{keyRedisHost: ""}},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Contains(t, err.Error(), keyRedisHost)
		})
	}
}

...
func main() {
	...
	if _, err := conf.LoadSecrets(context.Background(), daprClient, helper); err != nil {
		panic(err)
	}
	...
	if bc.Server == nil {
		panic("config: server section is required")
	}

	app, cleanup, err := wireApp(bc.Server, daprClient, logger)
	if err != nil {
